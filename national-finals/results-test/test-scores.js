/* ==========================================================================
   NATIONAL FINALS — TEST / MOCK SCORING DATA (INTERNAL STRESS-TEST ONLY)
   --------------------------------------------------------------------------
   THIS IS FAKE DATA. It powers the Day 1 Results and Day 2/Final Results
   tabs ONLY, and only while day1ResultsPublished / finalResultsPublished
   are manually flipped on for internal testing (see config.js — the
   default public state keeps both locked).

   IMPORTANT: this file is intentionally SEPARATE from data.js. data.js now
   holds the REAL current National Finals player roster (from the Players
   tab of the NF Scoring sheet); this file holds an independent, fictitious
   set of players + scores used only to stress-test the leaderboard UI
   (ranking, tie handling, mobile cards, division filters). Do not merge
   these two files, and do not treat the names/scores below as real.

   Field reference:
     name         - fictitious test player name (NOT a real roster entry)
     division     - "A" | "B" | "C" | "D" | "E"
     index        - WHS Handicap Index (test value)
   ...
   TIE-BREAK NOTE: `position` shown on the leaderboard is computed here
   client-side by ascending Day1Net/finalNet with an alphabetical
   ...
   apply that official tie-break and hand the site an already-resolved
